$(function() {
	//默认加载函数
	lion.web.AppInit();
 	var treegridId='#sys_theatrecategory_lists';
	var addForm=$('#addform');
	...
	handleVForm(addForm,submitForm);
	 ...
	 $('#btnAdd').on('click',function(){
		 addForm.reset();
	 	 addDialog.find('.modal-header h4 span').text('添加戏剧专场展演活动项目');
	 ...
	 $('#btnEdit').on('click',function(){
		 ...
		 if(!row){
 		 ...
 		 addForm.reset();
    	 addDialog.find('.modal-header h4 span').text('编辑戏剧专场展演活动项目');
...
function successAddFrm(data,arg,id){
  ...
  if(data!==null&&!(data.hasError)){
  	...
  	$('#basic').modal('toggle');
    $('#sys_theatrecategory_lists').treegrid('reload');
