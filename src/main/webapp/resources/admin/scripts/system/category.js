$(function() {
	//默认加载函数
	lion.web.AppInit();
 	var treegridId='#sys_category_lists';
	var addForm=$('#addform');
	...
	handleVForm(addForm,submitForm);
	 ...
	 $('#btnAdd').on('click',function(){
		 addForm.reset();
	 	 addDialog.find('.modal-header h4 span').text('添加比赛项目');
	 ...
	 $('#btnEdit').on('click',function(){
		 ...
		 if(!row){
 		 ...
 		 addForm.reset();
    	 addDialog.find('.modal-header h4 span').text('编辑比赛项目');
...
function successAddFrm(data,arg,id){
  ...
  if(data!==null&&!(data.hasError)){
  	...
  	$('#basic').modal('toggle');
    $('#sys_category_lists').treegrid('reload');
